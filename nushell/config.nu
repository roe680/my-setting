# config.nu
# Nushell-only conveniences and behavior settings

# Atuin integration (history)
$env.ATUIN_SESSION = (atuin uuid)
hide-env -i ATUIN_HISTORY_ID
$env.config.show_banner = false 
# Magic token to avoid recording keybinding-triggered commands
let ATUIN_KEYBINDING_TOKEN = $"# (random uuid)"

let _atuin_pre_execution = {||
    if ($nu | get -o history-enabled) == false { return }
    let cmd = (commandline)
...
# UI behavior (right prompt rendering)
...
source /home/roe/.config/nushell/scripts/helix.nu
